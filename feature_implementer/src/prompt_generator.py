...
def generate_prompt(
    ...
    template_path_obj = Path(template_path) if not isinstance(template_path, Path) else template_path
    ...
    if not template_content:
        ...
        raise ValueError(f"Template file {template_path} could not be read or is empty.")
    ...
    if jira_description:
        ...
        if jira_path.is_file():
            ...
            if not jira_description_content:
                logger.warning(f"Could not read Jira description file: {jira_description}")
    ...
    if additional_instructions:
        ...
        if instr_path.is_file():
            ...
            if not additional_instructions_content:
                logger.warning(f"Could not read instructions file: {additional_instructions}")
    ...
    final_prompt = template_content.format(
        ...
        additional_instructions=additional_instructions_final if additional_instructions_final else "N/A",
    )
