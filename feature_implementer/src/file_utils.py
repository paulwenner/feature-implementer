...
def get_file_tree(start_dirs: List[str], force_rescan: bool = False) -> Dict[str, Any]:
    ...
    if file_tree_cache.is_scanning():
        logger.info("File scan already in progress, returning cached data or empty dict")
                ...
                for item in sorted(start_path.rglob('*')):
                    ...
                    if item.is_file():
                        ...
                        for i, part in enumerate(parts):
                            if i == len(parts) - 1:
                                current_level[part] = (Config.WORKSPACE_ROOT / start_dir_name / relative_path).as_posix()
                            else:
                                if part not in current_level:
                                    current_level[part] = {}
                                if isinstance(current_level[part], str):
                                    logger.warning(f"Path conflict for {part} in {start_dir_name}")
                                    continue
                                current_level = current_level[part]
            except Exception as e:
                logger.error(f"Error scanning directory {start_path}: {e}", exc_info=True)
